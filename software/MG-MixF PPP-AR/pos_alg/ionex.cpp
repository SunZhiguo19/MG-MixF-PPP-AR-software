...
/* get index -----------------------------------------------------------------*/
int ionex::getindex(double value, const double *range)
{
...
void ionex::readionexdcb(FILE *fp, double *dcb, double *rms)
{
    int i,sat;
...
//                continue;
//            }
            dcb[sat-1]=m_mat_cal.str2num(buff, 6,10);
            rms[sat-1]=m_mat_cal.str2num(buff,16,10);
        }
        else if (strstr(label,"END OF AUX DATA")==label) break;
    }
}
/* read ionex header ---------------------------------------------------------*/
double ionex::readionexh(FILE *fp, double *lats, double *lons, double *hgts,
...
//    trace(3,"readionexh:\n");
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(label,"IONEX VERSION / TYPE")==label) {
            if (buff[20]=='I') ver=m_mat_cal.str2num(buff,0,8);
        }
        else if (strstr(label,"BASE RADIUS")==label) {
            *rb=m_mat_cal.str2num(buff,0,8);
        }
        else if (strstr(label,"HGT1 / HGT2 / DHGT")==label) {
            hgts[0]=m_mat_cal.str2num(buff, 2,6);
            hgts[1]=m_mat_cal.str2num(buff, 8,6);
            hgts[2]=m_mat_cal.str2num(buff,14,6);
        }
        else if (strstr(label,"LAT1 / LAT2 / DLAT")==label) {
            lats[0]=m_mat_cal.str2num(buff, 2,6);
            lats[1]=m_mat_cal.str2num(buff, 8,6);
            lats[2]=m_mat_cal.str2num(buff,14,6);
        }
        else if (strstr(label,"LON1 / LON2 / DLON")==label) {
            lons[0]=m_mat_cal.str2num(buff, 2,6);
            lons[1]=m_mat_cal.str2num(buff, 8,6);
            lons[2]=m_mat_cal.str2num(buff,14,6);
        }
        else if (strstr(label,"EXPONENT")==label) {
            *nexp=m_mat_cal.str2num(buff,0,6);
        }
...
/* read ionex body -----------------------------------------------------------*/
int ionex::readionexb(FILE *fp, const double *lats, const double *lons,
...
//    trace(3,"readionexb:\n");
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(label,"START OF TEC MAP")==label) {
            ...
            if (eph_T.str2time(buff,0,36,&time)) {
//                trace(2,"ionex epoch invalid: %-36.36s\n",buff);
                continue;
            }
            if (type==2) {
                for (i=nav->nt-1;i>=0;i--) {
                    if (fabs(eph_T.timediff(time,nav->tec[i].time))>=1.0) continue;
                    p=nav->tec+i;
                    break;
                }
            }
            ...
            lat   =m_mat_cal.str2num(buff, 2,6);
            lon[0]=m_mat_cal.str2num(buff, 8,6);
            lon[1]=m_mat_cal.str2num(buff,14,6);
            lon[2]=m_mat_cal.str2num(buff,20,6);
            hgt   =m_mat_cal.str2num(buff,26,6);
            ...
            for (m=0;m<n;m++) {
                ...
                if ((x=m_mat_cal.str2num(buff,m%16*5,5))==9999.0) continue;
...
//    trace(3,"combtec : nav->nt=%d\n",nav->nt);
    ...
    for (i=0;i<nav->nt-1;i++) {
        for (j=i+1;j<nav->nt;j++) {
            if (eph_T.timediff(nav->tec[j].time,nav->tec[i].time)<0.0) {
                tmp=nav->tec[i];
                nav->tec[i]=nav->tec[j];
                nav->tec[j]=tmp;
            }
    ...
    for (i=0;i<nav->nt;i++) {
        if (i>0&&eph_T.timediff(nav->tec[i].time,nav->tec[n-1].time)==0.0) {
...
* notes  : see ref [1]
*-----------------------------------------------------------------------------*/
void ionex::readtec(const char *file, nav_t *nav, int opt)
{
    FILE *fp;
...
//            trace(2,"ionex file format error %s\n",efiles[i]);
    ...
    /* P1-P2 dcb */
    for (i=0;i<MAXSAT;i++) {
        nav->cbias[i][0]=M_C*dcb[i]*1E-9; /* ns->m */
    }
}
/* interpolate tec grid data -------------------------------------------------*/
int ionex::interptec(const tec_t *tec, int k, const double *posp, double *value,
                     double *rms)
{
...
int ionex::iondelay(gtime_t time, const tec_t *tec, const double *pos,
    ...
    *delay=*var=0.0;
    ...
    for (i=0;i<tec->ndata[2];i++) { /* for a layer */
        ...
        if (opt&2) {
            ...
            rp=tec->rb/(tec->rb+hion)*sin(0.9782*(MM_PI/2.0-azel[1]));
            fs=1.0/sqrt(1.0-rp*rp);
        }
        if (opt&1) {
            /* earth rotation correction (sun-fixed coordinate) */
            posp[1]+=2.0*MM_PI*eph_T.timediff(time,tec->time)/86400.0;
        }
        /* interpolate tec grid data */
...
*-----------------------------------------------------------------------------*/
int ionex::iontec(gtime_t time, const nav_t *nav, const double *pos,
                  const double *azel, int opt, double *delay, double *var)
{
...
//          pos[0]*R2D,pos[1]*R2D,azel[0]*R2D,azel[1]*R2D);
    ...
    for (i=0;i<nav->nt;i++) {
        if (eph_T.timediff(nav->tec[i].time,time)>0.0) break;
    }
    if (i==0||i>=nav->nt) {
//        trace(2,"%s: tec grid out of period\n",time_str(time,0));
        return 0;
    }
    if ((tt=eph_T.timediff(nav->tec[i].time,nav->tec[i-1].time))==0.0) {
...
//              time_str(time,0),pos[0]*R2D,pos[1]*R2D,azel[0]*R2D,azel[1]*R2D);
    ...
    if (stat[0]&&stat[1]) { /* linear interpolation by time */
        a=eph_T.timediff(time,nav->tec[i-1].time)/tt;
...
*-----------------------------------------------------------------------------*/
int ionex::expath(const char *path, char *paths[], int nmax)
{
    int i,j,n=0;
    char tmp[1024];
#ifdef WIN32
...
*-----------------------------------------------------------------------------*/
double ionex::ionppp(const double *pos, const double *azel, double re,
                     double hion, double *posp)
{
    double cosaz,rp,ap,sinap,tanap;

    rp=re/(re+hion)*cos(azel[1]);
    ap=MM_PI/2.0-azel[1]-asin(rp);
    sinap=sin(ap);
    tanap=tan(ap);
    cosaz=cos(azel[0]);
    posp[0]=asin(sin(pos[0])*cos(ap)+cos(pos[0])*sinap*cosaz);

    if ((pos[0]> 70.0*D2R&& tanap*cosaz>tan(MM_PI/2.0-pos[0]))||
        (pos[0]<-70.0*D2R&&-tanap*cosaz>tan(MM_PI/2.0+pos[0]))) {
        posp[1]=pos[1]+MM_PI-asin(sinap*sin(azel[0])/cos(posp[0]));
    }
    else {
