...
const double  tidal_corr::args[][5]={
...
/*Initialize tidal correction parameters*/
void tidal_corr::initTidarVar()
{
...
*-----------------------------------------------------------------------------*/
int tidal_corr::readerp(const char *file, erp_t *erp)
{
    FILE *fp;
    erpd_t *erp_data;
    double v[14]={0};
...
void tidal_corr::setStationName(QString StationName /* = "" */)
{
    if(!StationName.isEmpty()) m_StationName = StationName.mid(0,4).trimmed();// Debug by xiaogongwei 2019.03.13
}

/*Incoming ocean data, erp file path, otherwise search from the current
 *  directory, otherwise it will not apply to ocean tides and extreme tide correction*/
void tidal_corr::setTideFileName(QString OCEANFileName,QString erpFileName)
{
    initTidarVar();
...
tidal_corr::~tidal_corr(void)
{
    if(m_erp_Data.data != NULL) free(m_erp_Data.data);
}

bool tidal_corr::readRepFile()
{
...
bool  tidal_corr::getErpV(gtime_t obsGPST,double *erpV)
{
    if(!geterp(&m_erp_Data,obsGPST,erpV)) return false;
    return true;
}

void tidal_corr::tide_pole(const double *pos, const double *erpv, double *denu)
{
...
void tidal_corr::getPoleTide(int Year,int Month,int Day,int Hours,int Minuts,double Seconds,double *pBLH,double *pTideENU)
{
    ...
    gtime_t obsGPST;
    pTideENU[0] = 0;pTideENU[1] = 0;pTideENU[2] = 0;
    tSeconds = YMD2GPSTime(Year,Month,Day,Hours,Minuts,Seconds,&tWeek);
    obsGPST = gpst2time(tWeek,Seconds);
    //Get erp parameter
    //if (!isgetErpV)
...
void tidal_corr::getSoildTide(int Year,int Month,int Day,int Hours,int Minuts,double Seconds,double *pXYZ,double *pTideENU,bool isElimate/* = false*/)
{
    pTideENU[0] = 0;pTideENU[1] = 0;pTideENU[2] = 0;
    ...
    if (isGetPos)//External incoming sun and moon data
    {
        ...
        getSunMoonPos(Year,Month,Day,Hours,Minuts,Seconds,sunpos,moonpos,&gmst);
    ...
    if (isElimate)
    {
        ...
        double  eN[3]= {-qSin(m_SationBLH[0])*qCos(m_SationBLH[1]),
                        -qSin(m_SationBLH[0])*qSin(m_SationBLH[1]),
                        qCos(m_SationBLH[0])};
    ...
    XYZ2ENU(pTideXYZ,pTideENU,pXYZ);
...
bool tidal_corr::readOCEANFile(QString  StationName,OCEANData &oceaData,QString  OCEANFileName)
{
    ...
    {
        QString infor = "readOCEANFile, Can not open BLQ file.";
...
void tidal_corr::getOCEANTide(int Year,int Month,int Day,int Hours,int Minuts,double Seconds,double *pXYZ,double *pTideENU,QString StationName)
{
    pTideENU[0]=0;pTideENU[1]=0;pTideENU[2]=0;
    if (!isReadOCEAN)
    ...
    if (LeapSeconds <= 0)//Jump seconds are only obtained once, which is a number greater than zero
        LeapSeconds = getLeapSecond(Year,Month,Day,Hours,Minuts,Seconds);
    ep[5]-=LeapSeconds;
...
void tidal_corr::getAllTideEffectENU(int Year,int Month,int Day,int Hours,int Minuts,double Seconds,double *pXYZ,double *pENU,double *psunpos/* =NULL */, double *pmoonpos /* = NULL */,double gmst /* = 0 */,QString StationName /* = "" */)
{
...
double tidal_corr::getAllTideEffect(int Year,int Month,int Day,int Hours,int Minuts,double Seconds,
                                     double *pXYZ,double *EA,double *psunpos, double *pmoonpos,double gmst,QString StationName)
{
    ...
    if (qAbs(m_SecondFlag - (Hours*3600+Minuts*60+Seconds)) > 0.3)//Two differences greater than 0.3s are considered to replace the epoch to recalculate the sun coordinates (a single epoch does not repeatedly calculate the sun coordinates)
    {
        if(psunpos&&pmoonpos)
            ...
            getSunMoonPos(Year,Month,Day,Hours,Minuts,Seconds,m_sunpos,m_moonpos,&m_gmst);//An epoch only needs to be calculated once, no need to calculate multiple times
...
void tidal_corr::getAlltidalData()
{
    readRepFile();//Read erp file to variable during initialization m_erp_Data
