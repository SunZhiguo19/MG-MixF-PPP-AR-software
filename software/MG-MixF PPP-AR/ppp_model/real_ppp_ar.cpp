#include "real_ppp_ar.h"


real_ppp_ar::real_ppp_ar(QObject *parent) : QThread(parent)
{
    // 注册自定义类型信号槽
    qRegisterMetaType<wigdet_transmit>("wigdet_transmit");
}

real_ppp_ar::~real_ppp_ar()
{

}


void real_ppp_ar::run(){
    ...
    if(opt_corr->mult_day)
    {
        for(int i=0;i<opt_corr->multi_file_jia.length();i++)
        {
            ...
            for(int j=0;j<multi_sta_OFileName.length();j++)
            {
                ...
                real_pppar_calculating(opt_corr,trans_para,dat_fil,sate_disp,OFileNamesList);
...
void real_ppp_ar::sysMain()
{

   this->start();

}
void real_ppp_ar::real_pppar_calculating(const corr_opt *opt_corr, para_trans *trans_para,file_dat *dat_fil,disply_satellite *sate_disp,QStringList OFileNamesList)
{
    stand_pos spp_p;
...
//            qDebug()<<"SimpleSPP_model start"<<endl;
            QString wirte_debug ="SimpleSPP_model start";
            spp_p.writeQdebug2Txt(wirte_debug);
...
//            qDebug()<<"SimpleSPP_model end"<<endl;
            wirte_debug ="SimpleSPP_model end";
            spp_p.writeQdebug2Txt(wirte_debug);
...
//                // debug by xiaogongwei 2020.04.24
            ...
            if(epoch_num==1023||epoch_num==8512)
            {
                 int debuga = 666;
                 debuga=0;
            }
...
//            qDebug()<<"qqqq0000"<<endl;
            wirte_debug ="qqqq0000";
            spp_p.writeQdebug2Txt(wirte_debug);
            if(!trans_para->backward_filter)
            {
                for (int i = 0;i < epochSatlitData.length();i++)
    ...
    //                tempSatlitData.Relativty = relative;
                    ...
                    if((PHASE_BIA[IF_fre0]==0.0||PHASE_BIA[IF_fre1]==0.0||CODE_BIA[IF_fre0]==0.0||CODE_BIA[IF_fre1]==0.0)&&dat_fil->bia_file_flag==0&&tempSatlitData.SatType!='R')
                    {
                        QString errorline;
                        ErrorMsg(errorline);
                    ...
                    else if((PHASE_BIA[IF_fre0]==0.0||PHASE_BIA[IF_fre1]==0.0)&&tempSatlitData.SatType!='R')
                    {
                        QString errorline;
                        ErrorMsg(errorline);
                        tempSatlitData.badMsg.append("PHASE_BIA is not " +errorline);
                        spp_p.allBadSatlitData.append(tempSatlitData);
                        continue;
                    }
    ...
    //                if(i==4&&epoch_num>100&&epoch_num<140)
    //                {
    //                    tempSatlitData.CodePP[0] = tempSatlitData.CodePP[0];
    //                    tempSatlitData.PhaseLL[0] = tempSatlitData.PhaseLL[0]+1;
    //                }
...
//            qDebug()<<"qqqq1222"<<endl;
            wirte_debug ="qqqq122222";
            spp_p.writeQdebug2Txt(wirte_debug);
            int Total_sat_fre=0;
            ...
            bool Mul_GNSS_AR = false;VectorXd  sd_a;
            ...
            {
                int Diff_sys_par_G[epochResultSatlitData.length()];
                int Diff_sys_par_C2[epochResultSatlitData.length()];
                int Diff_sys_par_C3[epochResultSatlitData.length()];
                int Diff_sys_par_R[epochResultSatlitData.length()];
                int Diff_sys_par_E[epochResultSatlitData.length()];
                int sys_length=0;int sat_length;
                for(int i=0;i<trans_para->sys_len.size();i++)
                   sys_length = sys_length+trans_para->sys_len[i];
                sd_a.resize(epochResultSatlitData.length()-sys_length);
                sd_a.setZero(epochResultSatlitData.length()-sys_length);
                for(int sys_n=0;sys_n<epochResultSatlitData.length();sys_n++)
                {
                    SatlitData SYS_SatlitData = epochResultSatlitData.at(sys_n);
                    switch (SYS_SatlitData.SatType) {
                    case 'G':
                        Diff_sys_par_G[sys_n] = Par_sat_epo[sys_n];
                        Diff_sys_par_C2[sys_n] =111 ;Diff_sys_par_C3[sys_n] =111 ;
                        Diff_sys_par_R[sys_n] =111 ;Diff_sys_par_E[sys_n] =111 ;
                        break;
                    case 'C':
                        if(SYS_SatlitData.PRN<=18)
                        {
                            Diff_sys_par_C2[sys_n] = Par_sat_epo[sys_n];
                            Diff_sys_par_G[sys_n] =111 ;Diff_sys_par_C3[sys_n] =111 ;
                            Diff_sys_par_R[sys_n] =111 ;Diff_sys_par_E[sys_n] =111 ;
                        }
                        else if(SYS_SatlitData.PRN>18)
                        {
                            Diff_sys_par_C3[sys_n] = Par_sat_epo[sys_n];
                            Diff_sys_par_C2[sys_n] =111 ;Diff_sys_par_G[sys_n] =111 ;
                            Diff_sys_par_R[sys_n] =111 ;Diff_sys_par_E[sys_n] =111 ;
                        }
                        break;
                    case 'R':
                        Diff_sys_par_R[sys_n] = Par_sat_epo[sys_n];
                        Diff_sys_par_C2[sys_n] =111 ;Diff_sys_par_C3[sys_n] =111 ;
                        Diff_sys_par_G[sys_n] =111 ;Diff_sys_par_E[sys_n] =111 ;
                        break;
                    case 'E':
                        Diff_sys_par_E[sys_n] = Par_sat_epo[sys_n];
                        Diff_sys_par_C2[sys_n] =111 ;Diff_sys_par_C3[sys_n] =111 ;
                        Diff_sys_par_R[sys_n] =111 ;Diff_sys_par_G[sys_n] =111 ;
                        break;
                    default:
                        break;
                    }
                ...
                for(int sys=0;sys<opt_corr->m_sys_str.length();sys++)
                {
                    char m_sys=' ';
                    if(opt_corr->m_sys_str[sys]=='G')
                        m_sys = 'G';
                    else if(opt_corr->m_sys_str[sys]=='C')
                        m_sys = 'C';
                    else if(opt_corr->m_sys_str[sys]=='R')
                        m_sys = 'R';
                    else if(opt_corr->m_sys_str[sys]=='E')
                        m_sys = 'E';
                    fix_flag = 0;
                    switch (m_sys) {
                    case 'G':
                        if(trans_para->sys_len[sys+BDS_2flag]==1)
                            Amb_res.ambiguity_search(epochResultSatlitData,P,Amb_res.SmoothSD,fix_flag,Diff_sys_par_G,ref_sat_prn,trans_para);
                        if(fix_flag==1)
                        {
                            for(sat_length=0;sat_length<Amb_res.SmoothSD.sd_a.rows();sat_length++)
                                if(Amb_res.SmoothSD.sd_a(sat_length)!=0)
                                    sd_a(sat_length) = Amb_res.SmoothSD.sd_a(sat_length);
                            Mul_GNSS_AR = true;
                        }
                        break;
                    case 'C':
                        ...
                        {
                            BDS_2flag++;
                            Amb_res.ambiguity_search(epochResultSatlitData,P,Amb_res.SmoothSD,fix_flag,Diff_sys_par_C2,ref_sat_prn,trans_para);
                            if(fix_flag==1)
                            {
                                for(sat_length=0;sat_length<Amb_res.SmoothSD.sd_a.rows();sat_length++)
                                    if(Amb_res.SmoothSD.sd_a(sat_length)!=0)
                        ...
                        fix_flag = 0;
                        if(trans_para->sys_len[sys+BDS_2flag]==1)
                            Amb_res.ambiguity_search(epochResultSatlitData,P,Amb_res.SmoothSD,fix_flag,Diff_sys_par_C3,ref_sat_prn,trans_para);
                        if(fix_flag==1)
                        {
                            for(sat_length=0;sat_length<Amb_res.SmoothSD.sd_a.rows();sat_length++)
                                if(Amb_res.SmoothSD.sd_a(sat_length)!=0)
                    ...
                    case 'R':
                        if(trans_para->sys_len[sys+BDS_2flag]==1)
                            Amb_res.ambiguity_search(epochResultSatlitData,P,Amb_res.SmoothSD,fix_flag,Diff_sys_par_R,ref_sat_prn,trans_para);
                        if(fix_flag==1)
                        {
                            for(sat_length=0;sat_length<Amb_res.SmoothSD.sd_a.rows();sat_length++)
                                if(Amb_res.SmoothSD.sd_a(sat_length)!=0)
                    ...
                    case 'E':
                        if(trans_para->sys_len[sys+BDS_2flag]==1)
                            Amb_res.ambiguity_search(epochResultSatlitData,P,Amb_res.SmoothSD,fix_flag,Diff_sys_par_E,ref_sat_prn,trans_para);
                        if(fix_flag==1)
                        {
                            for(sat_length=0;sat_length<Amb_res.SmoothSD.sd_a.rows();sat_length++)
                                if(Amb_res.SmoothSD.sd_a(sat_length)!=0)
...
//            SD_AR_Lamda(epochResultSatlitData,Amb_res.SmoothSD,P,X,fix_flag,SDflag,ref_sat_prn);//szg debug 1230 SDflag0504
            ...
            wirte_debug ="szg 0828";
            spp_p.writeQdebug2Txt(wirte_debug);
            if(fix_flag == 3||Mul_GNSS_AR)//szg 0828
            {
                VectorXd flo_b,fix_b;
                wirte_debug ="fix_flag == 1";
                spp_p.writeQdebug2Txt(wirte_debug);
                ...
                if(!opt_corr->GNSS_AR_Select)
                    ...
                    fix_b = flo_b - Amb_res.SmoothSD.Fix_Qab * Amb_res.SmoothSD.Fix_Qa.inverse() * (sd_a);//Fixed solution
...
//            qDebug()<<"qqqq4444"<<endl;
            wirte_debug ="changexyz(result)";
            spp_p.writeQdebug2Txt(wirte_debug);
